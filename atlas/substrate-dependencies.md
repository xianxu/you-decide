...
The motivating bug: the controversy map (`data/controversies/2026/CA.md`) was drafted against 30 candidate profiles. Nine more profiles were later added. None of the original 30 changed — so a last-modified-time check on the map's inputs passes — yet the map was stale, because its *input set grew* and nothing recomputed it. That failure mode is the reason this doc exists. See **Freshness rules** below.
...
|---|---|---|---|
| Source registry | `data/sources/<US\|state>.md` | root | human-curated, rarely changes |
...
| Election manifest | `data/elections/<year>/<date>-<state>-<type>.md` | root-ish (derived from SoS filings, an external source) | research subagents |
