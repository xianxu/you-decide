...
The data the algorithm operates on. All shared substrate lives under `data/` at the repo root (committed); user-private deliberation lives in the user's private dir, resolved by `scripts/private-dir.sh` (outside this repo — never committed).

## Public substrate (this repo)

### `data/candidates/<year>/<state>/<office>/<slug>.md`
...
### `data/elections/<year>/<YYYY-MM-DD>-<state>-<scope>.md`
...
Composed at runtime: `data/sources/US.md` always loaded; `data/sources/<state>.md` loaded for the state being processed. Sub-jurisdiction lists (county, city) added inline in those files as needed.
