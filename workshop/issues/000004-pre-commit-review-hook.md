...
**Fresh-eyes code review (subagent, 2026-05-31): verdict SHIP, no Critical.** Built ~20 adversarial fixtures; confirmed every malformed-input path fails closed. Addressed before PR: (a) the one fail-*permissive* path — an **unclosed** frontmatter fence read the body as metadata; fixed the awk to emit the block only when a closing fence is seen (no fence → all fields `<missing>` → block); (b) CRLF tolerance (strip trailing `\r` so a legit CRLF file parses, still fail-closed if malformed); (c) added the reviewer's recommended test cases (body-only `review:`, duplicate `reviewed-by`, empty file, unclosed fence) — harness now 20/20. Not changed: the cosmetic all-zeros-base success-message SHA display (shows `0000000..` rather than the empty-tree hash — exit codes/diff identical); case-sensitivity of stack equality (exact inequality is as-specified; compound/case normalization deferred to M4). Lesson logged in `workshop/lessons.md`. `sdlc pr` → you-decide PR #4.

**Phase E finding — the CI merge-check mechanism (ariadne #52) is broken in CI.** PR #4's `merge-check` workflow fired (good) but failed with `scripts/run-merge-checks.sh: No such file or directory` (exit 127). Root cause: the runner is a sibling symlink (`scripts/run-merge-checks.sh -> ../../ariadne/scripts/run-merge-checks.sh`) that resolves locally (where `../ariadne` exists) but NOT in the GitHub Actions checkout, which clones only you-decide. The *workflow shim* is already seeded as a real file precisely "because GitHub Actions does not discover symlinked workflow files" — the same CI-isolation reasoning was never applied to the symlinked runner. The gate LOGIC is correct + proven (20/20 harness + real-data + local runner); only the CI *delivery* of the base-layer runner is broken. ariadne is public, so cross-repo checkout is viable. This is an ariadne #52 base-layer fix (affects all derivatives).

**Resolved (operator chose: clone ariadne via bootstrap.sh — the single-script mechanism every ariadne-style repo uses).** Added a CI step `BOOTSTRAP_CLONE_ONLY=1 ./bootstrap.sh` before the merge-checks step: it clones the upstream peer chain as siblings (transitively; handles deep chains generically) and skips the heavy `make bootstrap` handoff. Peer URLs derive from origin (public ariadne → no token). Guarded on `bootstrap.sh` presence so the root repo is unaffected. **Verified live on PR #4: CI green** — `bootstrap: cloning peer 'ariadne'` → `✓ 10-review-gate.sh` → `✓ 20-cross-stack-gate.sh` → `✓ all 2 check(s) passed`. The full mechanism (CI clones base layer → symlinked runner resolves → both gates run over the PR range) is validated end-to-end (= #53 Phase E). **Remaining:** backport the workflow step to the ariadne #52 *seed template* so future derivatives are seeded with the fix (existing seeded derivatives using CI need the patch applied — seed is write-once).
